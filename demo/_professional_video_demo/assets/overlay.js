 ...
 * by `_professional_video_demo`'s recorded walkthrough videos.
 * WHY: Kept in its own file (rather than embedded as a Python string) so
 * the Python driver code stays focused on orchestration/pacing only —
 * single-responsibility split between "what the browser renders" (this
 * file) and "when/what it's told to show" (`core/overlay.py`). Also
 * makes each half far easier to review/edit independently without
 * touching a large embedded string literal.
